...
import RatingSummaryModel from "@/components/servers/rating-summary-model";
import { fetchOneLLM } from "@/actions/yoooo";
import { fetchAllReviews, fetchReviewsStatsByType, ReviewType } from "@/actions/reviews";
import { Progress } from "@/components/ui/progress";
import AllReviews from "@/components/client/all-reviews";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: llm.llm_picture ? [{ url: llm.llm_picture }] : [],
      type: 'website',
    },
    twitter: {
      ...
      title: `${llm.name} - Language Model Reviews & Ratings`,
      description: llm.description,
      images: llm.llm_picture ? [llm.llm_picture] : [],
    }
  }
}
...
export default async function ModelPage({
  ...
  const reviewStats = await fetchReviewsStatsByType({ id: llm.id, type: ReviewType.MODEL });
